...
function depends_on(reaction, sources, checked) {
	...
	if (reaction.deps !== null) {
		for (const dep of reaction.deps) {
			...
			if (
				(dep.f & DERIVED) !== 0 &&
				depends_on(/** @type {Derived} */ (dep), sources, checked)
			) {
	...
	#show_pending_snippet() {
		...
		if (this.#main_effect !== null) {
			...
			this.#offscreen_fragment.append(
				/** @type {TemplateNode} */ (this.#pending_anchor),
			);
				...
				this.#failed_effect = this.#run(() => {
					...
					try {
						...
						invoke_error_boundary(
							error,
							/** @type {Effect} */ (this.#effect.parent),
						);
						return null;
					} finally {
						this.#is_creating_fallback = false;
...
function internal_set(source, value) {
	if (!source.equals(value)) {
		...
		if ((source.f & DERIVED) !== 0) {
			...
			set_signal_status(
				source,
				(source.f & CONNECTED) !== 0 ? CLEAN : MAYBE_DIRTY,
			);
		}
...
function handle_event_propagation(event) {
	var owner_document = /** @type {Node} */ (this).ownerDocument;
	...
	if (handled_at) {
		...
		if (
			at_idx !== -1 &&
			(this === document || this === /** @type {any} */ (window))
		) {
	...
	try {
		...
		event.__root = this;
		// @ts-expect-error remove proxy on currentTarget
...
class Spring {
	...
	constructor(value, options = {}) {
		...
		if (typeof options.stiffness === "number")
			this.#stiffness.v = clamp$1(options.stiffness, 0, 1);
		if (typeof options.damping === "number")
			this.#damping.v = clamp$1(options.damping, 0, 1);
 ...
 */
var pointerPositionFromEvent = (event, box) => {
 ...
 */
var derivePointerState = (box, position) => {
 ...
 */
var toNormalizedXY = (value) => {
	var _a, _b;
	var tuple;
	if (Array.isArray(value)) {
		tuple = [
			(_a = value[0]) !== null && _a !== void 0 ? _a : 0.5,
			(_b = value[1]) !== null && _b !== void 0 ? _b : 0.5,
		];
 ...
 */
var closenessToEdge = (half, dx, dy) => {
...
var root = from_html(
	`<div part="container"><!> <!> <div part="tilt"><!></div></div>`,
);
...
function HoverTilt($$anchor, $$props) {
	...
	const resetSpringOptions = () => {
		if (initialSpringOptions.stiffness && initialSpringOptions.damping) {
			...
			position.stiffness =
				initialTiltSpringOptions.stiffness ?? initialSpringOptions.stiffness;
			position.damping =
				initialTiltSpringOptions.damping ?? initialSpringOptions.damping;
		}
	};
	...
	const scaleValue = user_derived(
		() => 1 + (scaleFactor() - 1) * activation.current,
	);
	...
	const pointerState = user_derived(() =>
		get(pointerBox)
			? derivePointerState(get(pointerBox), {
	...
	const rotationY = user_derived(
		() => ROTATION * (tiltFactorY() ?? tiltFactor()),
	);
  ...
  `,
	...
	onMount(() => {
		...
		if (!get(isWebComponent) && !get(componentId)) {
			set(
				componentId,
				`hover-tilt-${Math.random().toString(36).substring(2, 9)}`,
			);
		}
	});
	...
	template_effect(() => {
		...
		set_class(
			div,
			1,
			`hover-tilt-container ${containerClass() ?? ""}`,
			"svelte-1g9r0vt",
		);
		set_style(
			div,
			`${get(dynamicStyleVariables)} ${staticProperties} ${containerStyle()}`,
		);
